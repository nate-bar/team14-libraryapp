...
const UsingFetch = () => {
      ...
      .then((data) => {
        if (Array.isArray(data)) {
          const updatedData = data.map(item => ({
            ...item,
            GenreName: genres.find(g => g.id === item.GenreID)?.name || "N/A"
          }));
  ...
  const handleGenreFilterChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedGenre = e.target.value; // This is the GenreID from the dropdown
    setGenreFilter(selectedGenre);
  
    console.log("Selected Genre ID:", selectedGenre); // LOG

    if (selectedGenre === "") {
      console.log("Filtering: All Genres");
      const filtered = items.filter((item) => typeFilter === "" || item.TypeName === typeFilter);
      console.log("Filtered Items:", filtered);
      setFilteredItems(filtered);
    } else {
      console.log("Filtering: Specific Genre");
      const filtered = items.filter(
        (item) =>
          (typeFilter === "" || item.TypeName === typeFilter) &&
          (item.TypeName === "Device" || item.Genre === genres.find(g => g.id === parseInt(selectedGenre))?.name)
      );
      console.log("Filtered Items:", filtered);
      setFilteredItems(filtered);
    }
  ...
  const getGenreName = (genreName: string | null) => genreName || "N/A";
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {filteredItems.map((item) => (
            ...
            >
              ...
              <p className="text-sm text-gray-700">
                <strong>Genre:</strong> {getGenreName(item.GenreName)}
              </p>
            </div>
          ))}
        </div>
