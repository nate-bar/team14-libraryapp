...
import "./reports.css";
import MostBorrowedItems from "./MostBorrowedItems";
...
const Report = () => {
  ...
  useEffect(() => {
    ...
    if (selectedReport === "userBorrowedItemsReport") {
  ...
  const updateStats = (dataToProcess: BorrowedItemRecord[]) => {
    ...
    const overdue = dataToProcess.filter(
      (item) =>
        item.BorrowStatus === "Borrowed" && new Date(item.DueDate) < today
    ).length;

    const totalFines = dataToProcess.reduce(
      (sum, item) => sum + (item.FineAccrued || 0),
      0
    );

    const bookCount = dataToProcess.filter((item) =>
      item.ItemType?.toLowerCase().includes("book")
    ).length;

    const mediaCount = dataToProcess.filter((item) =>
      item.ItemType?.toLowerCase().includes("media")
    ).length;

    const deviceCount = dataToProcess.filter((item) =>
      item.ItemType?.toLowerCase().includes("device")
    ).length;
  ...
  const applyFilters = () => {
    ...
    if (itemTypeFilter !== "all") {
      if (itemTypeFilter === "book") {
        filtered = filtered.filter(
          (record) => record.ISBN !== null && record.ISBN !== undefined
        );
        ...
        filtered = filtered.filter(
          (record) =>
            record.DeviceType !== null && record.DeviceType !== undefined
        );
      }
    }
  ...
  const renderReport = () => {
    switch (selectedReport) {
      case "mostBorrowedItemsReport":
        return <MostBorrowedItems />;
      case "userBorrowedItemsReport":
        return renderUserBorrowedItemsReport();
      case "bookDetailsReport":
        return <p>Book Details Report (Not implemented yet)</p>;
      default:
        return <p>Select a report to view</p>;
    }
  };

  // Render the User Borrowed Items Report
  const renderUserBorrowedItemsReport = () => {
    return (
      <>
        {/* Filters */}
                    ...
                    >
                      ...
                      <td
                        className={`border px-4 py-2 ${
                          isOverdue ? "font-bold text-red-700" : ""
                        }`}
                      >
  ...
  return (
    <div className="report-container">
      ...
      <div className="mb-6">
        ...
        >
          <option value="userBorrowedItemsReport">
            User Borrowed Items Report
          </option>
