...
const EventPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEventDetails = async () => {
      ...
      try {
        ...
        if (!eventResponse.ok) {
          throw new Error(
            `HTTP error! status: ${eventResponse.status} fetching event details`
          );
        }
        ...
        if (!itemsResponse.ok && itemsResponse.status !== 404) {
          // Check for non-404 errors
          throw new Error(
            `HTTP error! status: ${itemsResponse.status} fetching event items`
          );
        }
  ...
  if (error) {
    return (
      <div className="error-message">Error loading event details: {error}</div>
    );
  }

  if (!eventId || !event) {
    return (
      <div className="error-message">Invalid event ID or event not found.</div>
    );
  }
      ...
      {eventItems.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {eventItems.map((item) => (
            <div
              key={item.ItemID}
              className="card rounded-lg shadow-md p-4 bg-white"
            >
        ...
        <p className="text-gray-500">
          {event.EventName} event has no items! Check back later!
        </p>
      )}
    </div>
  );
